...
export function matchStationName(value: string): { station: KochiMetroStation; score: number } | null {
  ...
  const matches = kochiMetroStations.flatMap((station) => [station.name, station.shortName ?? "", ...station.aliases]
...
export const calculateStationIntervals = (from: KochiMetroStation, to: KochiMetroStation) => Math.abs(from.routeOrder - to.routeOrder);
export const calculateRouteDistance = (from: KochiMetroStation, to: KochiMetroStation) =>
  Number(Math.abs(from.cumulativeDistanceKm - to.cumulativeDistanceKm).toFixed(1));

export function validateDetectedRoute(fromText: string, toText: string, confidence: number, threshold = 0.72) {
  const from = matchStationName(fromText); const to = matchStationName(toText);
